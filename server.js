...
// var HAAPI = require('./requestHAAPI')
...
app.get('/search', function (req, res, next) {
  ...
  if (!empty(value)) {
    request.get({
      uri: 'https://www.als.ogcio.gov.hk/lookup?q=' + encodeURIComponent(value),
      method: 'GET',
      json: true,
      headers: {
      ...
      // Error Checking
      if (error || empty(body.SuggestedAddress)) {
      ...
      for (var i = 0; i < body.SuggestedAddress.length; i++) {
        ...
        var lat = address.PremisesAddress.GeospatialInformation[0].Latitude
        var lng = address.PremisesAddress.GeospatialInformation[0].Longitude
        // Testing
        // console.log(lat, lng);
      ...
      // Testing
      // console.log('coord len , eng_add len', coord.length, eng_address.length)
...
/*
app.get('/haapi', function (req, res, next) {
  // Testing HA API
  HAAPI()
})
*/
